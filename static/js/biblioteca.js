...
async function renderizarListaUnica() {
    ...
    try {
        ...
        itens.forEach(item => {
            ...
            // Verifica se pode gerar questões (PDF/TXT/JSON/Markdown de anexos)
            const tiposGeracaoIA = ['pdf', 'txt', 'json', 'md', 'markdown'];
            const podeGerarIA = item.origem === 'anexo' && tiposGeracaoIA.includes((item.tipo || '').toLowerCase());
            ...
            const acoesHtml = item.origem === 'anexo'
                ? podeGerarIA
                    ...
                    : `<button class="btn-secondary" disabled title="Apenas PDF, TXT, JSON e Markdown podem gerar questões">🚫 Não gera questões (${item.tipo.toUpperCase()})</button>`
                : `
